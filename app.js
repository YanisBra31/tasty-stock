/* ═══════════════════════════════════════════════════════════
   TASTY STOCK — app.js  (version Supabase / online)
   Toutes les opérations de données sont async via supabase.js
═══════════════════════════════════════════════════════════ */
...
let currentResto   = null;   // ID uuid du restaurant actif
let editingId      = null;
...
// Cache en mémoire de session (rafraîchi au besoin)
let _restos    = [];
let _stock     = [];   // stock du restaurant actif
let _transfers = [];
let _users     = [];
...
// ═══════════════════════════════════════════════════════════
//  BOOT — vérification de session existante
// ═══════════════════════════════════════════════════════════
async function boot() {
  try {
...
async function doLogin() {
  ...
  // Supabase Auth utilise des emails — si l'utilisateur tape un pseudo
  // sans @, on ajoute un domaine fictif (pratique si vous créez les
  // comptes vous-même via le dashboard Supabase)
...
function renderRestoCards() {
  ...
  grid.innerHTML = _restos.map((r, i) => {
    ...
    // Stats depuis le cache s'il existe, sinon affiche des tirets
...
async function goChoose() {
  // Rafraîchit la liste des restos depuis la DB
  setLoading(true);
  try {
    invalidateCache('restos');
    _restos = await cachedRestos();
...
async function openResto(id) {
  ...
  setLoading(true);
  try {
    _stock = await cachedStock(id);
  } catch (err) {
    toast('Erreur de chargement du stock', 'err');
    _stock = [];
  } finally {
    setLoading(false);
  }
...
function renderDashboard() {
  ...
  al.innerHTML = alertItems.length
    ? alertItems.map(i => {
        const s = getStatus(i);
        const badgeClass = s === 'out' || s === 'exp' ? 'badge-rupture' : 'badge-low';
        const badgeLabel = s === 'out' ? 'Rupture' : s === 'exp' ? 'Expiré' : 'Stock bas';
        const dotColor   = s === 'out' || s === 'exp' ? 'var(--pink)' : 'var(--orange)';
        return `<div class="alert-item">
          <div class="ai-dot" style="background:${dotColor}"></div>
...
function rowHTML(i, withActions = false) {
  ...
  const actions = withActions
    ? `<td><div class="row-actions">
        <button class="btn-icon" onclick="openModal('edit','${i.id}')">✏️</button>
        <button class="btn-icon del" onclick="deleteItem('${i.id}')">🗑</button>
      </div></td>`
    : '<td></td>';
  return `<tr>
...
async function saveItem() {
  ...
  try {
    ...
    if (activePage === 'page-alertes') renderAlertes();
  } catch (err) {
...
function loadTransferItems() {
  ...
  // Stock du restaurant source : depuis le cache si dispo, sinon _stock
...
async function doTransfer() {
  ...
  try {
    ...
    // Invalide le cache des stocks concernés
    invalidateCache('transfers');
    _cache.stock[fromId] = null;
    _cache.stock[toId]   = null;
    // Si on était sur le restaurant source, rechargement
    if (fromId === currentResto) {
      _stock = await cachedStock(currentResto);
    }
    _transfers = await cachedTransfers();
...
async function initComparaison() {
  ...
  setLoading(true);
  try {
    // Charge le stock de tous les restos
    const all = await Promise.all(_restos.map(async r => {
      const stock = await cachedStock(r.id);
      return { r, stock };
    }));
    renderComparaison(all);
  } catch (err) {
    content.innerHTML = `<div class="empty-state"><div class="es-icon">❌</div><p>Erreur de chargement</p></div>`;
  } finally {
    setLoading(false);
  }
}
...
function renderComparaison(all) {
  const content = document.getElementById('compare-content');
  if (!all.length) {
    content.innerHTML = `<div class="empty-state"><div class="es-icon">🏪</div><p>Aucun restaurant</p></div>`;
    return;
  }
...
function renderUsers() {
  ...
  if (!_users.length) {
    list.innerHTML = `<div class="empty-state"><div class="es-icon">👥</div><p>Aucun utilisateur</p></div>`;
    return;
  }
  ...
  list.innerHTML = `<div class="users-table-wrap">
    ${_users.map((u, i) => `
      <div class="user-row-item">
        ...
        <div class="uri-info">
          <div class="uri-name">${esc(u.name)}</div>
          <div class="uri-username">${esc(u.role)}</div>
        </div>
        <span class="role-badge ${roleClass[u.role]||'role-employe'}">${esc(u.role)}</span>
...
function openUserModal(id = null) {
  ...
  // Note : pour créer un user, Supabase envoie un email de confirmation.
  // L'email sert d'identifiant, on affiche le champ correspondant.
  if (id) {
...
async function saveUser() {
  ...
  try {
    if (editingUserId) {
      // Met à jour le profil uniquement (pas l'email/password en anon key)
      ...
      toast('Invitation envoyée par email ✓', 'ok');
    }
    invalidateCache('users');
    _users = await cachedUsers();
    closeUserModal();
...
function renderRestosAdmin() {
  ...
  if (!_restos.length) {
    list.innerHTML = `<div class="empty-state"><div class="es-icon">🏪</div><p>Aucun restaurant</p></div>`;
    return;
  }
    ...
    ${_restos.map((r, i) => {
      ...
      return `<div class="resto-admin-card">
        ...
        <div class="rac-actions">
          ...
          ${_restos.length > 1 ? `<button class="btn danger-btn" style="font-size:11px" onclick="deleteResto('${r.id}')">🗑 Supprimer</button>` : ''}
        </div>
      </div>`;
    }).join('')}
  </div>`;
...
function closeRestoModal() {
  document.getElementById('resto-modal-overlay').classList.remove('open');
  editingRestoId = null;
}
function closeRestoModalBg(e) {
  if (e.target.id === 'resto-modal-overlay') closeRestoModal();
}
...
async function saveResto() {
  ...
  try {
    if (editingRestoId) {
      await sbUpdateResto(editingRestoId, { name, location, color });
      toast('Restaurant mis à jour', 'ok');
    } else {
      await sbCreateResto(name, location, color);
      toast('Restaurant créé', 'ok');
    }
    invalidateCache('restos');
    _restos = await cachedRestos();
    closeRestoModal();
    const activePage = document.querySelector('.page.active')?.id;
    if (activePage === 'page-restaurants') renderRestosAdmin();
    renderRestoCards();
  } catch (err) {
    toast('Erreur : ' + err.message, 'err');
  } finally {
...
async function deleteResto(id) {
  ...
  setLoading(true);
  try {
    await sbDeleteResto(id);
    invalidateCache();
    _restos = await cachedRestos();
    toast('Restaurant supprimé', 'info');
    renderRestosAdmin();
    renderRestoCards();
  } catch (err) {
    toast('Erreur : ' + err.message, 'err');
